...
def model_IPTG_BP(t, U_flat, shape, params, w, receiver_coordinates):
    ...
    #na, ka, nr, kr= [1.58646428, -0.6564445, 7.99974394, -2.14317698]
    #na, ka, nr, kr= [2.60754348, -1.8366539, 4.75478292, -2.16703204]

    D, alpha_a, beta_a, K_Ia, n_Ia, K_laca, alpha_r, beta_r, K_Ir, n_Ir, K_lacr, alpha_g, beta_g, K_A, n_A, K_R, n_R, d_R = params
    ...
    activator = ((alpha_a*(1- (U_grid[0]/K_Ia)**n_Ia)) / (1 + (U_grid[0]/K_Ia) ** n_Ia + K_laca) + beta_a) * U_grid[2]

    repressor = ((alpha_r*(1- (U_grid[0]/K_Ir)**n_Ir)) / (1 + (U_grid[0]/K_Ir) ** n_Ir + K_lacr) + beta_r) * U_grid[2]
    ...
    gfp = ((alpha_g * (U_grid[4]/K_A)**n_A) / (1 + (U_grid[4]/K_A)**n_A + (U_grid[5]/K_R)**n_R) + beta_g + (U_grid[5]/K_A)*d_R) * U_grid[2]
...
def run_experiment(params, IPTG_conc, plot = False):
    ...
    sim_ivp = sim_ivp.y.reshape(6, n_rows, n_cols, t_points)
...
if __name__ == '__main__':
    ...
    if threshold:
        ...
        IPTG_concs = [0., 1., 2.5, 5., 10, 50., 100., 500.]

        #bounds = ([1, -5, 1, -5, -4, -9, -3], [6, 3, 6, 3, -1, 2, 2])   #na, ka, nr, kr, D, min, max
        ...
        bounds = ([ -4, 1, 1, -8, 1, 2, 1, 1, -8, 1, 2, -5, -5, 1, 1, 0, 1, 0], [ -1, 5, 5, -4, 5, 6, 5, 5, -4, 5, 6, 2, 2, 5, 5, 4, 6, 10])   # D, min, max
        #bounds = ([-4], [-1]) #D
        if particle_swarm:
            print('script started')
            ...
            optimizer = ps.single.GlobalBestPSO(n_particles=1000, dimensions=18, options=options, bounds=bounds)

            # Perform optimization
            cost, pos = optimizer.optimize(vector_objective, iters=100, verbose=True)
        elif brute_force:
            ...
            #error: 1.1227631381262595
            params =  [-1.90224107,  3.79478742,  2.17381188, - 4.60087062,  3.27679157,  3.60851053,
                     3.77789303,  1.03863772, - 5.99118008 , 1.9381769 ,  4.14253144, - 3.43914396,
                     - 2.97358441,  4.98271216 , 1.69916175,  1.52613632 , 2.88134031,  6.08157807]

            #D, alpha_a, beta_a, K_Ia, n_Ia, K_laca, alpha_r, beta_r, K_Ir, n_Ir, K_lacr, alpha_g, beta_g, K_A, n_A, K_R, n_R, d_R = params



            concs = np.linspace(0,50/1000, 100)

            #plt.plot(concs, hill_BP(concs, na, ka, nr, kr, min, max))
            plt.figure()
            run_all_experiments(params, plot=True)
            plt.show()
